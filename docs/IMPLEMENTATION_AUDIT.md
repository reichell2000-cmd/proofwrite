...
| 교사 Dashboard               | 미완료                    | 테스트 완료 | 실제 제출 목록, 전체 글 우선, 접힌 점수·추천, 읽기 확인, 구체적 피드백      |
...
- 교사 생성→학생 한글/붙여넣기/수정→재접속→제출→교사 2대목 확인·구체적 피드백→학생 후속 수정→교사 조회 통과.
...
[비평과 수정 예시](TEACHER_REVIEW.md)에 검토 근거를 기록했습니다. 목표·기준을 함께 보여주고, 교사 기본 탭은 전체 글, 점수는 접힌 보조 지표로 바꿨습니다. 실제 본문의 대목·강점의 이유·다음 수정 한 가지가 있어야 피드백을 전달할 수 있습니다. 학생은 별도 후속 연습과 질문을 저장할 수 있습니다.

브라우저 시험은 중간 피드백 비공개, 본문에 없는 인용 거부, 읽기/피드백 요건 우회 거부, 후속 응답 권한/CSRF/충돌/재시도, 제출 원문·이벤트 보존, 데스크톱/모바일 화면을 추가로 검증합니다. 이전 과제의 새 필드 생략도 허용합니다.

![학생 피드백과 다음 시도](screenshots/student-feedback.png)
